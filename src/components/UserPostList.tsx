...
const UserPostList = ({ username, initialPosts }: UserPostListProps) => {
  ...
  async function loadMorePosts() {
    const next = page + 1;
    const newPosts = await fetchUserPosts({ page: next, username });
    if (newPosts?.length) {
      setPage(next);
      setPosts((oldPosts: Post[] | undefined) => [...(oldPosts?.length ? oldPosts : []), ...newPosts]);
      if (newPosts?.length < postTake) {
        // Less than postTake post remained, meaning there are definitely not more to load
