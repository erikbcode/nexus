...
interface SubnexusPostListProps {
  communityName: string;
  initialPosts: Post[];
}

const SubnexusPostList = ({ communityName, initialPosts }: SubnexusPostListProps) => {
  ...
  async function loadMorePosts() {
    const next = page + 1;
    const newPosts = await fetchCommunityPosts({ page: next, subnexusName: communityName });
    if (newPosts?.length) {
      setPage(next);
...
export default SubnexusPostList;
